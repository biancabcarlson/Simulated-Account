...
├── package.json        # includes both fixture records when packaged
└── README.md
```
...
Open `index.html` in a browser to see the same data rendered as a tabbed
dashboard (home, payment methods, ACH history, crypto activity, login/IP log,
KYC, security, documents). The Home tab starts with an ATO case brief and
direct next-step links, while Security includes an incident replay and a
computed risk score.

### Add evidence to the timeline

Every account-activity, ACH, crypto, login/IP, and security-review item is
sent to Timeline Builder automatically as it renders — text, detected
date/time, time-known status, and source/context all queue themselves in
the browser-local `investigatorSuiteTimelineInbox`; nothing to click, no
review step. Each item shows a small "✓ In timeline" badge to confirm it's
synced. Sync is idempotent (deduped by a hash of the event's content), so
re-sorting, paging, or reopening the page never creates duplicate entries.
Timeline Builder imports the events automatically and stores custom events
under `investigatorSuiteTimelineEvents`; if a date/time or source needs
correcting, that's done in Timeline Builder's own edit mode ("Select
event").
...
The Security tab includes a "Case Activity" feed (`investigatorSuiteActivity` in `localStorage`) that logs every case decision made in *any* tool in the suite — e.g. rejecting a document in Documents Folder shows up here without opening that tool. Resolved risk flags are persisted under `investigatorSuiteResolvedRiskFlags`, so reopening the dashboard does not make an investigator repeat the same decision. This is a real shared log, not just cross-links: it updates live across open tabs via the `storage` event.
