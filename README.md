...
an **Add to timeline** action. Clicking it sends the evidence — text,
detected date/time, time-known status, and source/context — straight to
Timeline Builder through the browser-local `investigatorSuiteTimelineInbox`
queue; there's no review step and no clipboard paste required. The button
briefly confirms with "Added ✓". Timeline Builder imports the event
automatically and stores custom events under
`investigatorSuiteTimelineEvents`; if a date/time or source needs
