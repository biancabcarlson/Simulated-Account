...
A "🔒 Privacy Mode" toggle (top right) blurs every field tagged as PII — name, account/card/routing numbers, wallet addresses, phone, email, residential address, DOB, IP addresses, and the ID-verification modal — without hiding the page structure, so the layout can be screen-shared or demoed without exposing (simulated) customer data. State is shared across every tool in the suite via `localStorage` (`investigatorSuitePrivacyMode`), so toggling it here also toggles it in Case Doc Tracker, Case Calculator, etc.

### Case Role

A role selector (Analyst / Investigator / Manager) sits next to Privacy Mode, shared the same way (`investigatorSuiteRole`). Analyst is read-only for case decisions; Investigator and Manager can additionally resolve a fraud flag under Security → Flagged Events. Below Investigator/Manager, "Mark resolved" is disabled with a tooltip rather than hidden, so every role can still see what the action would do.

### Case Activity

The Security tab includes a "Case Activity" feed (`investigatorSuiteActivity` in `localStorage`) that logs every case decision made in *any* tool in the suite — e.g. rejecting a document in Case Doc Tracker shows up here without opening that tool. This is a real shared log, not just cross-links: it updates live across open tabs via the `storage` event.
